...
function computeRelativeHmrPath(base) {
 ...
 * @returns Vite plugin
 */
export function reactiveViewPlugin(options = {}) {
    ...
    return {
        ...
        configResolved(config) {
            const base = normalizeBasePath(config.base);
            const serverConfig = config.server;
            if (serverConfig.hmr === false) {
                return;
            }
            const existingHmr = typeof serverConfig.hmr === "object" && serverConfig.hmr !== null
                ? { ...serverConfig.hmr }
                : {};
            const currentPath = typeof existingHmr.path === "string" ? existingHmr.path : undefined;
            if (isDefaultHmrPath(currentPath)) {
                const resolvedPath = computeRelativeHmrPath(base);
                if (currentPath !== resolvedPath) {
                    log(`Normalized Vite HMR path for base ${base} -> ${resolvedPath}`);
                }
 ...
 */
export function invalidateLoaders(server, routes, type = "modified") {
    server.ws.send({
        type: "custom",
        event: "reactive-view:loader-update",
        data: {
